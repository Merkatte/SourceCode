...
public class cards6 : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
{
    Shuffle shuffled;
    ...
    void IEndDragHandler.OnEndDrag(PointerEventData eventData)
    {
        ...
        if (chkObj.tile_chk == true)
        {
            ...
            if (heron_tile.on_tile == false && gameManager.isGoldEnough(card_Loader.Loaded_Info[9].cost))
            {
                soundManager.playSound("Card_Put");
                Vector3 vec = new Vector3(heron_tile.transform.position.x, heron_tile.transform.position.y, -2);
                GameObject instance = objectManager.MakeObj("Watermelon");
